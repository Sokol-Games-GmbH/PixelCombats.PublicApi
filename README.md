...
### Всплывающие сообщения (попапы)
- Включаются/выключаются на стороне режима при инициализации через `IRoomAPI.PopupsEnable`.
- По умолчанию — выключено (`false`). Изменять можно только в момент инициализации.
