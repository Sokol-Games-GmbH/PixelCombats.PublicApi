...
**VB**
``` VB
ReadOnly Property FillQuad As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ FillQuad {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract FillQuad : IApiProp<bool, IBuildContext> with get
```
