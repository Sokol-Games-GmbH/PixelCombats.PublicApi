...
**VB**
``` VB
ReadOnly Property QuadChangeEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ QuadChangeEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract QuadChangeEnable : IApiProp<bool, IBuildContext> with get
```
