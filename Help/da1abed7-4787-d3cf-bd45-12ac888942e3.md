...
**VB**
``` VB
ReadOnly Property GenMapEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ GenMapEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract GenMapEnable : IApiProp<bool, IBuildContext> with get
```
