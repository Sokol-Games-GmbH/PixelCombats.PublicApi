...
**VB**
``` VB
ReadOnly Property BalkLenChange As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ BalkLenChange {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract BalkLenChange : IApiProp<bool, IBuildContext> with get
```
