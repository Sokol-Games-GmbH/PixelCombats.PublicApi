# Parameters - поле


параметры игрового режима



## Definition
**Пространство имён:** <a href="4d3fbb36-c354-8f6e-a905-e9536bb5c956">PixelCombats.DTO.Room.GameMode</a>  
**Сборка:** PixelCombats.DTO (в PixelCombats.DTO.dll) Версия: 1.0.0.0 (1.0.0.0)

**C#**
``` C#
public GameModeParametersSet Parameters
```
**VB**
``` VB
Public Parameters As GameModeParametersSet
```
**C++**
``` C++
public:
GameModeParametersSet Parameters
```
**F#**
``` F#
val mutable Parameters: GameModeParametersSet
```



#### Значение поля
<a href="f397412c-4f58-7c2c-9b35-f7df90d33d37">GameModeParametersSet</a>

## См. также


#### Ссылки
<a href="8cfb61fb-9136-66d6-4652-00bca13fbf61">GameModeMeta - </a>  
<a href="4d3fbb36-c354-8f6e-a905-e9536bb5c956">PixelCombats.DTO.Room.GameMode - пространство имён</a>  
