...
**VB**
``` VB
ReadOnly Property SetSkyEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ SetSkyEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract SetSkyEnable : IApiProp<bool, IBuildContext> with get
```
