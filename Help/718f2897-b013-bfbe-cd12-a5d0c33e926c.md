...
**VB**
``` VB
ReadOnly Property CollapseChangeEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ CollapseChangeEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract CollapseChangeEnable : IApiProp<bool, IBuildContext> with get
```
