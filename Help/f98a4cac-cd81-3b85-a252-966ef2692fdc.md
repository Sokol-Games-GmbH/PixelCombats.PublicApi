# ChangeMapAuthorsEnable - свойство
...
**VB**
``` VB
ReadOnly Property ChangeMapAuthorsEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ ChangeMapAuthorsEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract ChangeMapAuthorsEnable : IApiProp<bool, IBuildContext> with get
```
