...
**VB**
``` VB
ReadOnly Property RenameMapEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ RenameMapEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract RenameMapEnable : IApiProp<bool, IBuildContext> with get
```
