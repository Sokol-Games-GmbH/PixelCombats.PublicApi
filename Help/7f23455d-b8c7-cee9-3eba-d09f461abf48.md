...
**VB**
``` VB
ReadOnly Property BuildModeEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ BuildModeEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract BuildModeEnable : IApiProp<bool, IBuildContext> with get
```
