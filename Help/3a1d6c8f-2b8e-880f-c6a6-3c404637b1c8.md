...
**VB**
``` VB
ReadOnly Property LoadMapEnable As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ LoadMapEnable {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract LoadMapEnable : IApiProp<bool, IBuildContext> with get
```
