...
**VB**
``` VB
ReadOnly Property Pipette As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ Pipette {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract Pipette : IApiProp<bool, IBuildContext> with get
```
