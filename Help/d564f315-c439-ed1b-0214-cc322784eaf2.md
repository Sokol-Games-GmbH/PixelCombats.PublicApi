...
**VB**
``` VB
ReadOnly Property RemoveQuad As IApiProp(Of Boolean, IBuildContext)
	Get
```
**C++**
``` C++
property IApiProp<bool, IBuildContext^>^ RemoveQuad {
	IApiProp<bool, IBuildContext^>^ get ();
}
```
**F#**
``` F#
abstract RemoveQuad : IApiProp<bool, IBuildContext> with get
```
